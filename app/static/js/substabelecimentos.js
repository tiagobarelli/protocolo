...
function mostrarAutocompleteClientes(resultados) {
  ...
  for (var i = 0; i < resultados.length; i++) {
    (function(row) {
      ...
      item.addEventListener('click', function() {
        var isAdvogado = !!row['field_7430'];
        adicionarCliente(row.id, nome, isAdvogado);
        document.getElementById('clienteInput').value = '';
        fecharAutoList('clienteAutoList');
      });
      lista.appendChild(item);
...
function renderizarChipCliente(id, nome, advogado) {
...
function aplicarBloqueioSubstabelecimento(deveTravar) {
...
function salvarSubstabelecimento() {
  if (substabelecimentoRowId === null || !bloqueioWidget) {
    executarSalvarSubstabelecimento();
    return;
  }
...
function executarSalvarSubstabelecimento() {
  var livro = document.getElementById('livroInput').value.trim();
  var pagina = document.getElementById('paginaInput').value.trim();
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (window.criarBloqueioRegistro) {
    bloqueioWidget = window.criarBloqueioRegistro({
      tabelaId: CONFIG.tables.substabelecimentos,
      badgeContainerId: 'bloqueioBadge',
      botaoContainerId: 'bloqueioBotao',
      obterRowId: function() { return substabelecimentoRowId; },
      aoAplicarBloqueio: aplicarBloqueioSubstabelecimento
    });
  }
  ...
  // Abertura via query params (ex: /substabelecimentos?livro=150&pagina=025)
